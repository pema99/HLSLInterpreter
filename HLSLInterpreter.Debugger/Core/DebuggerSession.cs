...
public sealed class DebuggerSession
{
    ...
    public static DebuggerSession Record(
        string code,
        int warpX, int warpY,
        int groupOffsetX = 0, int groupOffsetY = 0,
        string entryPoint = "main",
        Action<HLSLRunner>? configureGlobals = null,
        HLSLParserConfig? parserConfig = null)
    {
        ...
        try
        {
            ...
            configureGlobals?.Invoke(runner);
            ...
            runner.ProcessCode(code, parserConfig ?? new HLSLParserConfig(), out var diagnostics, out _);
            ...
            {
                var threadArg = DebugDispatch.BuildThreadArg(wx, wy, threadCount, groupOffsetX, groupOffsetY);
                result = DebugDispatch.CallEntryPoint(runner, threadArg, entryPoint);
            }
        }
        catch (Exception ex)
        {
